...
usair.inla2 <- inla(usair.formula1, data = usair, control.compute = list(dic = TRUE, cpo =TRUE),
	control.fixed = list(mean.intercept = 100, prec.intercept = 10^(-2), mean = list(negtemp = 2, wind = -3, default =0), prec = 1), 
...
knitr::kable(usair.inla1.pred$summary.fitted.values[(nrow(usair)+1):nrow(usair.combined),], caption = '拟合情况', align='c')
```
...
knitr::kable(round(usair.inla4$summary.fixed, 4),caption = '固定效应情况', align='c')

knitr::kable(round(usair.inla4$summary.hyperpar, 4), caption = '
	超参数情况', align='c')
```
...
knitr::kable(head(painrelief), caption = '
	painrelief数据', align='c')
```
